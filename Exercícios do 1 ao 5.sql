...
Select tb_cliente.nome FROM tb_cliente WHERE exists
( SELECT nome FROM tb_loja WHERE nome = 'Loja Unidade Minas Gerais' ) 

-- Para cada produto existente diga quais vendas foram realizadas e quantos itens foram perdidos
Select tb_venda.data_venda, (Select count (*) from tb_itens where tb_venda.pk_venda = tb_itens.fk_venda) 'Itens perdidos'
from tb_venda 

-- Para cada produto mostre as notas fiscais de venda para os itens que corresponderem aos seus produtos
Select tb_NotaFiscal.fk_venda, (Select count (*) from tb_itens where tb_NotaFiscal.pk_notafiscal = tb_itens.fk_produto) 'Nota Fiscal' 
from tb_NotaFiscal

-- Agrupe todos os vendedores e suas vendas
select count (*) 'Loja Unidade Minas Gerais', tb_venda.fk_vendedor as tb_cliente 
from tb_venda 
GROUP BY
		 tb_venda.fk_vendedor;

-- Para cada vendedor mostre quantas vendas foram realizadas
SELECT
		c.nome, COUNT(v.pk_venda) AS qtd_venda
FROM 
		tb_vendedor AS c, tb_venda AS v
WHERE 
		c.pk_vendedor=v.fk_vendedor
GROUP BY 
		c.nome; 